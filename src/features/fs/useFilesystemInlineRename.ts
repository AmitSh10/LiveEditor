...
import { createFileFS, createFolderFS, renameNodeFS, deleteNodeFS } from '../workspace/filesystemWorkspaceSlice';
...
/**
 * Get the path of a node from root
...
/**
 * Filesystem-aware inline rename/create state + business rules.
 * Creates real files and folders on disk.
 */
export function useFilesystemInlineRename({
	root,
	rootId,
	projectId,
	dispatch,
	...
	const confirmDelete = useCallback(
		async (node: FSNode) => {
			...
			const path = getNodePath(root, node.id, rootId);
			if (path === null) {
				alert('Could not determine file path');
				return;
			}

			await deleteNodeFS(dispatch, projectId, path, node.type === 'folder');
			setEditing(null);
			afterCommit?.();
		},
		[afterCommit, dispatch, projectId, root, rootId]
	);
	...
	const commitEditing = useCallback(async () => {
		...
		if (editing.mode === 'newFile' || editing.mode === 'newFolder') {
			...
			const parentPath = getNodePath(root, parent.id, rootId);
			...
			if (editing.mode === 'newFolder') {
				...
				await createFolderFS(dispatch, projectId, parentPath || '', raw);
				setEditing(null);
				afterCommit?.();
				return;
			}
			...
			await createFileFS(dispatch, projectId, parentPath || '', raw);
			setEditing(null);
			afterCommit?.();
			return;
		}
		...
		if (target.type === 'folder') {
			...
			const oldPath = getNodePath(root, target.id, rootId);
			const parentPath = getNodePath(root, parent.id, rootId);
			const newPath = parentPath ? `${parentPath}/${raw}` : raw;

			if (oldPath !== null) {
				await renameNodeFS(dispatch, projectId, oldPath, newPath, true);
			}
			setEditing(null);
			afterCommit?.();
			return;
		...
		if (oldPath !== null) {
			await renameNodeFS(dispatch, projectId, oldPath, newPath, false);
		}
		setEditing(null);
		afterCommit?.();
	}, [afterCommit, dispatch, editing, projectId, root]);
